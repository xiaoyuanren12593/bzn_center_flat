...
/**
  ...
  * Date:2019/11/26
  * Time:17:06
  * describe: 神盾-赔付概况和方案工种表
  **/
object DmAegisEmployerClaimInfoAndPlanWorkTypeDetail extends SparkUtil with Until with MysqlUntil{
  ...
  def getAegisEmployerPlanAndWorkTypeInfoData(sqlContext:HiveContext): DataFrame = {
    ...
    val res = sqlContext.sql(
      """
        ...
        |from
        |(
        |   select channel_id,channel_name,insure_company_short_name,profession_type,sku_charge_type,sku_coverage,sku_ratio,
        |   cast(case when sku_charge_type = '2' then sku_price/12 else sku_price end as decimal(14,4)) as sku_price,work_type,bzn_work_risk,
