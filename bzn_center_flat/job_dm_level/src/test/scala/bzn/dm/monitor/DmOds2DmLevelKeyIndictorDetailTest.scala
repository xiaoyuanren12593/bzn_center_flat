...
object DmOds2DmLevelKeyIndictorDetailTest extends SparkUtil with DataBaseUtil{
  def main(args: Array[String]): Unit = {
    ...
    dmData._2.printSchema()

//    saveASMysqlTable(dmData._1: DataFrame, dmMonitorTable: String, SaveMode.Overwrite,user106:String,pass106:String,mysqlDriver:String,url106:String)
  ...
  /**
    ...
    * @param sqlContext
    */
  def getOds2DmlevelKeyData(sqlContext:HiveContext):(DataFrame,DataFrame) = {

    /**
      * dm层结果
      */
    val dmData = dmLevelData(sqlContext:HiveContext)

    /**
      * ods-dw层关键指标结果
      */
    val odsAndDwData = dwLevelData(sqlContext)
  ...
  def dwLevelData(sqlContext:HiveContext)= {
    ...
    val dwData = sqlContext.sql(
      """
        ...
        |left join
        |(
        |select policy_code,sum(case when day_id = regexp_replace(substr(cast(now() as string),1,10),'-','') then count else 0 end) as curr_insured,sum(case when day_id = regexp_replace(substr(cast(now() as string),1,10),'-','') then 1 else 0 end) as insured_index,
