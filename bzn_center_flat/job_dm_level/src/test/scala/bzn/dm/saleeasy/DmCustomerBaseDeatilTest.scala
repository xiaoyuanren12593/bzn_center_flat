...
*/ object DmCustomerBaseDeatilTest extends SparkUtil with Until with MysqlUntil {
  ...
  def CustomerBase(hqlContext: HiveContext): Unit = {
    ...
    //企业级别的结案案件数
    val finalLevel = hqlContext.sql("SELECT holder_name_salve,count(case_type) as final_case_counts,insure_company_name as company_name3  from policyAndClaimTemp WHERE  case_status IS NOT NULL GROUP BY holder_name_salve,insure_company_name")
