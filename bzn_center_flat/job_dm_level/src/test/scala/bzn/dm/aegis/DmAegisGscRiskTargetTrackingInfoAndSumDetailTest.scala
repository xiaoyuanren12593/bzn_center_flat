...
/**
  ...
  **/
object DmAegisGscRiskTargetTrackingInfoAndSumDetailTest extends SparkUtil with DataBaseUtil{
...
//    saveASMysqlTable(res1: DataFrame, tableName1: String, SaveMode.Overwrite,user106:String,pass106:String,driver:String,url106:String)
  ...
  def getAegisGscRiskTargetTrackingInfoData(sqlContext:HiveContext): (DataFrame, DataFrame) = {
    ...
    val hisDiskPartitionData: Seq[hisDiskPartition] = Seq(
      ...
      hisDiskPartition(	"漳州吉胜劳务派遣有限公司",	"A"	),
   ...
   sqlContext.sql(
      """
        ...
        |left join
        |(
        |-- 每个保单的累计赔付金额、死亡赔付、伤残赔付、死亡案件数、伤残案件数、案件数、2020年的赔付金额
