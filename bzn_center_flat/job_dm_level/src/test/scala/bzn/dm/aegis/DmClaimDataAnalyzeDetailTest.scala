...
/**
  ...
  **/
object DmClaimDataAnalyzeDetailTest extends SparkUtil with DataBaseUtil with Until{
  ...
  /**
    ...
    * @param sqlContext
    */
  def getNewChargePremiumData(sqlContext:HiveContext) = {
    ...
    val replenishPremiumData = sqlContext.sql("select policy_id as policy_id_slave,cast(sum(case when sku_day_price is null then 0 else sku_day_price end) as decimal(14,10)) as replenish_premium from temp_sku_price where day_id <= regexp_replace(subStr(cast(now() as string),1,10),'-','') group by policy_id")
