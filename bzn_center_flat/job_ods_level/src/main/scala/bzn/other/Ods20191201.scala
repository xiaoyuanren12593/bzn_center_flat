...
object Ods20191201 extends SparkUtil with Until with MysqlUntil {
  ...
  def wubaData(hiveContext: HiveContext): DataFrame ={
    ...
    //读取数据
    val frame = hiveContext.sql("select insured_name,insured_cert_no," +
      "insured_mobile,null as policy_code,null as policy_id, start_date, " +
      "end_date,create_time,update_time,product_code,sku_price,business_line,years " +
      "from odsdb.ods_all_business_person_base_info_detail_temp where business_line = '58'")
