...
/**
  ...
  **/
object OdsProposalPersonDetail extends SparkUtil with MysqlUntil with Until{
 ...
 // ;
  ...
  def getPersonData(sqlContext:HiveContext) = {
    ...
    val tProposalSubjectPersonMasterBznbusi: DataFrame = readMysqlTable(sqlContext: SQLContext, tableName1: String,user106:String,pass106:String,driver:String,url106:String)
      ...
      .where("start_date > '2019-11-24' and start_date < '2019-12-02'")
    ...
    sqlContext.sql("truncate table odsdb.ods_proposal_detail")
    temp.show()
    temp.repartition(10).write.mode(SaveMode.Overwrite).saveAsTable("odsdb.ods_proposal_detail")

    //sqlContext.sql("select * from res").repartition(1).write.mode(SaveMode.Overwrite).format("csv").save("/xing/data/OdsProposalPersonDetail/detail")
   ...
   //result.repartition(1).write.mode(SaveMode.Overwrite).format("csv").save("/xing/data/OdsProposalPersonDetail/detail1")
  }
}
