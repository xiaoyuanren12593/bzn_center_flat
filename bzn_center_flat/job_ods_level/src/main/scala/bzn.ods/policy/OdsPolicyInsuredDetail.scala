...
object OdsPolicyInsuredDetail extends SparkUtil with Until{
  def main(args: Array[String]): Unit = {
    ...
    hiveContext.sql("truncate table odsdb.ods_policy_insured_detail")
    res.repartition(10).write.mode(SaveMode.Append).saveAsTable("odsdb.ods_policy_insured_detail")
    res.repartition(1).write.mode(SaveMode.Overwrite).parquet("/dw_data/ods_data/OdsPolicyInsuredDetail")
  ...
  def twoOdsPolicyInsuredDetail(sqlContext:HiveContext) ={
    ...
    val bPolicySubjectPersonMasterBzncen = sqlContext.sql("select * from sourcedb.b_policy_subject_person_master_bzncen")
      .selectExpr("id as master_id","policy_no as master_policy_no","name as insured_name","cert_type as insured_cert_type",
        ...
        "industry_name as industry","work_type","job_company","company_name","company_phone","status","start_date as insured_start_date",
    ...
    val res = InsuredData.join(bPolicyBzncen,InsuredData("master_policy_no") ===bPolicyBzncen("policy_no"),"leftouter")
      .selectExpr("getUUID() as id","clean(master_id) as insured_id","clean(cast(policy_id as String)) as policy_id","clean(insurance_policy_no) as policy_code",
        ...
        "clean(work_type_new) as work_type",
        "clean(job_company) as job_company",
        "clean(company_name) as company_name","clean(company_phone) as company_phone","cast(clean(insured_status) as int) as insured_status",
