...
object OdsPreservationDetail extends SparkUtil with Until{
  def main(args: Array[String]): Unit = {
    ...
    hiveContext.sql("truncate table odsdb.ods_preservation_detail")
    res.repartition(10).write.mode(SaveMode.Append).saveAsTable("odsdb.ods_preservation_detail")
    res.repartition(1).write.mode(SaveMode.Overwrite).parquet("/dw_data/ods_data/OdsPolicyDetail/OdsPreservationDetail")
  ...
  def twoPreservationDetail(sqlContext:HiveContext) ={
    ...
    val bPolicyPreserveBznprdOne: DataFrame = readMysqlTable(sqlContext,"b_policy_preservation_bzncen")
      .selectExpr("id as preserve_id","policy_no","effective_date","inc_dec_order_no" ,"inc_revise_no as add_batch_code","inc_revise_premium as add_premium","inc_revise_sum as add_person_count",
        "dec_revise_no as del_batch_code","dec_revise_premium as del_premium","dec_revise_sum as del_person_count","preservation_type as preserve_type","pay_status","create_time","update_time")
    ...
    val tep_five = tep_four.join(bPolicyPreserveBznprdTemp,"temp_inc_dec_order_no").map(x => {
      ...
      if(addPersonCount == 0 && delPersonCount > 0){
        ...
        if(createTime!=null && createTime.toString.length >18){
          preserveEffectDate = createTime.toString.substring(0,10).replaceAll("-","")
        }else{
          preserveEffectDate = null
        }
      }
    ...
    val resTemp = sqlContext.sql("select * from bPolicyInfoMasterInfoTemp")
      .selectExpr("preserve_id","insurance_policy_no as policy_code","policy_id","inc_dec_order_no" ,"add_batch_code",
