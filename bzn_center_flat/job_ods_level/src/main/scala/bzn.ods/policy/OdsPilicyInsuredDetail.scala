...
/**
  ...
  * describe: 人员清单明细表
  **/
object OdsPilicyInsuredDetail extends SparkUtil with Until{
...
//    oneDate.write.format("parquet").mode("overwrite").save("/xing/data/odsPolicyInsuredDetail")
  ...
  def twoOdsPolicyInsuredDetail(sqlContext:HiveContext) ={
    ...
    /**
      * 保单状态和在保状态   原表中1在职 2离职  ods层  0在职 1离职  保单状态 1在保2减员
      */
  ...
  def oneOdsPolicyInsuredDetail(sqlContext:HiveContext) ={
    ...
    val resTemp = insuredDataPolicyOne.unionAll(insuredDataPolicyTwo)
      .selectExpr("getUUID() as id","master_id as insured_id","policy_id","policy_code","name_new as insured_name ","insured_cert_type","insured_cert_no","birthday","gender","insured_mobile","industry","work_type_new as work_type","company_name","company_phone","status as insured_status","insure_policy_status as policy_status","start_date",
