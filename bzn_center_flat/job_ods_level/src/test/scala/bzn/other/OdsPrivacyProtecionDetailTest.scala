...
object OdsPrivacyProtecionDetailTest extends SparkUtil with Until {
  ...
  def PrivacyProtecion(hiveContext: HiveContext): DataFrame = {
    ...
    val officialTemp =
      hiveContext.sql("select " +
        ...
        "insured_mobile, MD5(insured_mobile) as insured_mobile_md5,business_line,years from odsdb.ods_all_business_person_base_info_detail where business_line = 'official'")
    ...
    //增量数据
    val officialRes = official.join(odsMd5MessageOfficial, 'id === 'id_salve, "leftouter")
      .select("id", "id_salve", "insured_cert_no", "insured_cert_no_md5", "insured_mobile", "insured_mobile_md5", "business_line", "years")
      .where("id_salve is null")
    ...
    //增量数据
    val weddingRes = wedding.join(odsMd5MessageWedding, 'id === 'id_salve, "leftouter")
      .select("id", "id_salve", "insured_cert_no", "insured_cert_no_md5", "insured_mobile", "insured_mobile_md5", "business_line", "years")
      .where("id_salve is null")
    ...
    //增量数据
    val interRes = inter.join(odsMd5MessageInter, 'id === 'id_salve, "leftouter")
      .select("id", "id_salve", "insured_cert_no", "insured_cert_no_md5", "insured_mobile", "insured_mobile_md5", "business_line", "years")
      .where("id_salve is null")

    val res = officialRes.unionAll(weddingRes).unionAll(interRes)
