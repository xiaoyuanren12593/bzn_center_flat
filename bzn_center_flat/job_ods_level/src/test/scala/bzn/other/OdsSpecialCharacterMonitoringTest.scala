...
import bzn.other.OdsEarlyWarningMonitoring.{HivePecialCharacter, MysqlPecialCharacter, MysqlRateRules, readMysqlTable, sparkConfInfo}
import bzn.other.numerationMonitoringTest.sparkConfInfo
...
object OdsSpecialCharacterMonitoringTest extends SparkUtil with Until with MysqlUntil {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //source保单
    //监控2.0保单特殊字符
    val policyTwo = MysqlPecialCharacter(sqlContext,"sourced", "b_policy_bzncen", "insurance_policy_no", "mysql.username.106", "mysql.password.106", "mysql.driver", "mysql.url.106")
    //监控1.0保单特殊字符
  ...
  def MysqlPecialCharacter(SQLContext: SQLContext,houseName:String, tableName: String, fieldType: String, user: String, pass: String, driver: String, url: String): DataFrame = {
    ...
    val resTemp: DataFrame = readMysqlTable(SQLContext, table, username, password, drivers, urls)
      ...
      }).toDF("monitoring_house", "monitoring_table", "monitoring_field","monitoring_text", "monitoring_level")
    val res = resTemp.
      selectExpr(
        "monitoring_house",
        "monitoring_table",
  ...
  def MysqlRateRules(SQLContext: SQLContext, tableName: String, fieldType: String, user: String, pass: String, driver: String, url: String): DataFrame = {
    ...
    //规则一 小数类的值超过1 预警
    val resTemp: DataFrame = readMysqlTable(SQLContext, table, username, password, drivers, urls)
      .selectExpr(s"cast($field as double) as field").distinct
      .map(x => {
