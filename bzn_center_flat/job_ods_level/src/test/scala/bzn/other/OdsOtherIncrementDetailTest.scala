...
import bzn.other.OdsOtherIncrementDetail.{clean, dateDelectOneMonth, readMysqlTable}
...
*/ object OdsOtherIncrementDetailTest extends SparkUtil with Until with MysqlUntil {
  ...
  def OdsOtherToHive(hiveContext: HiveContext): DataFrame = {
    ...
    val data1 = readMysqlTable(hiveContext, "open_other_policy", "mysql.username.103",
      "mysql.password.103", "mysql.driver", "mysql.url.103.bzn_open_all")
      .where("substring(cast(month as string),1,7) = substring(cast(getNow() as string),1,7)")
      .selectExpr("policy_id", "insured_name", "insured_cert_no", "insured_mobile", "policy_no", "start_date", "end_date", "create_time", "update_time",
        "product_code", "null as sku_price", "'inter' as business_line",
        "substring(cast(month as string),1,7) as months")

    // 读取接口当月数据
    val data2 = hiveContext.sql("select policy_id as policy_id_salve,years,business_line as business_line_salve from odsdb.ods_all_business_person_base_info_detail where business_line = 'inter' and substring(cast(getNow() as string),1,7) = years")
    ...
    val data4 = readMysqlTable(hiveContext, "open_other_policy", "mysql.username.103",
      "mysql.password.103", "mysql.driver", "mysql.url.103.bzn_open_all")
    ...
    // 读取hive上个月数据
    val data5 = hiveContext.sql("select policy_id as policy_id_salve,years,business_line as business_line_salve from odsdb.ods_all_business_person_base_info_detail where business_line = 'inter' and substring(cast(dateDelectOneMonth(getNow()) as string),1,7) = years")

    //拿到上个月数据的增量
    val data6 = data4.join(data5, 'policy_id === 'policy_id_salve, "leftouter")
      .selectExpr("policy_id", "policy_id_salve", "insured_name", "insured_cert_no", "insured_mobile", "policy_no", "start_date", "end_date", "create_time", "update_time", "product_code", "sku_price", "business_line", "months")
      .where("policy_id_salve is null")

    //合并数据
    val res1 = data3.selectExpr("insured_name", "insured_cert_no", "insured_mobile", "policy_no", "policy_id", "start_date", "end_date", "create_time", "update_time", "product_code", "sku_price", "business_line", "months as years")
    val res2 = data6.selectExpr("insured_name", "insured_cert_no", "insured_mobile", "policy_no", "policy_id", "start_date", "end_date", "create_time", "update_time", "product_code", "sku_price", "business_line", "months as years")
