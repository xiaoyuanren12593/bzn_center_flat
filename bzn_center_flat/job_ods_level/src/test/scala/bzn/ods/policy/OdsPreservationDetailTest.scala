...
//    onePreservationDetail(hiveContext)
  ...
  def twoPreservationDetail(sqlContext:HiveContext) ={
    ...
    val bPolicyBzncen = readMysqlTable(sqlContext,"b_policy_bzncen")
      .selectExpr("id as policy_id","policy_no as b_policy_no")


    /**
      * 读取被保人表
      */
    val bPolicyPreservationSubjectPersonMasterBzncenOne: DataFrame = readMysqlTable(sqlContext,"b_policy_preservation_subject_person_master_bzncen")
    ...
    val bPolicyInfo = bPolicyPreserveBznprd.join(bPolicyBzncen,bPolicyPreserveBznprd("temp_policy_no") ===bPolicyBzncen("b_policy_no"),"leftouter")
      .selectExpr("preserve_id","policy_no","temp_policy_no","policy_id","inc_dec_order_no","temp_inc_dec_order_no","add_batch_code","add_premium","add_person_count","del_batch_code","del_premium","del_person_count","preserve_type","pay_status","create_time","update_time")
    ...
    val bPolicyInfoMasterInfo = bPolicyInfo.join(bPolicyPreservationSubjectPersonMasterBzncen,Seq("temp_policy_no", "temp_inc_dec_order_no"),"leftouter")
      .selectExpr("preserve_id","policy_no","policy_id","inc_dec_order_no" ,"add_batch_code","add_premium","add_person_count","del_batch_code","del_premium","del_person_count","pre_start_date","pre_end_date","preserve_type","pay_status","create_time","update_time")
    ...
    val resTemp = sqlContext.sql("select * from bPolicyInfoMasterInfoTemp")
      .selectExpr("preserve_id","policy_no","policy_id","inc_dec_order_no" ,"add_batch_code","add_premium","add_person_count","del_batch_code","del_premium","del_person_count","preserve_type","pay_status","create_time","update_time")
    val res = resTemp.join(tep_five,resTemp("inc_dec_order_no") ===tep_five("temp_inc_dec_order_no"),"leftouter")
      .selectExpr("getUUID() as id","preserve_id","policy_id","policy_code","'4' as preserve_status","add_batch_code","add_premium","add_person_count","del_batch_code","del_premium","del_person_count","preserve_effect_date","preserve_type","pay_status","create_time","update_time","getNow() as dw_create_time")
    res.show()
    res
  }
  ...
  def onePreservationDetail(sqlContext:HiveContext) ={
    ...
    val res = plcPolicyPreserveBznprdRes.join(bPolicyBzncen,plcPolicyPreserveBznprdRes("policy_code")===bPolicyBzncen("insurance_policy_no"),"leftouter")
      .selectExpr("getUUID() as id","preserve_id","case when id is null then policy_id else id end as policy_id","policy_code","status as preserve_status","add_batch_code","add_premium","add_person_count","del_batch_code","del_premium","del_person_count","preserve_effect_date","preserve_type","getDefault() as pay_status","create_time","update_time","getNow() as dw_create_time")
    res.show()
    res
  }
