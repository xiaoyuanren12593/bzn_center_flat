...
*   2.投保人情况
  ...
  * c)
  **/
object CPersonHighInfoCusType extends SparkUtil with Until with HbaseUtil{
  ...
  def getOldInfo(sqlContext:HiveContext,dwPolicyDetailInc:DataFrame,odsPolicyDetail:DataFrame,policyHolderData:DataFrame,odsPolicyInsuredDetail:DataFrame,
    ...
    val centRes = res.selectExpr("cert_no","cus_type")
    toHBase(centRes,tableName,columnFamily,rowKeyName)
    val highRes = res.selectExpr("cert_no","become_curr_cus_time","last_cus_type")
    toHBase(highRes,tableName,columnFamily1,rowKeyName)

//    res.rdd.repartition(1).saveAsTextFile("/xing/data/2019-8-1/old_cus_type")
    res
  }
...
//    incHolderResult.rdd.repartition(1).saveAsTextFile("/xing/data/2019-8-1/incHolderResult")
    ...
    toHBase(incInsuredResult,tableName,columnFamily,rowKeyName)
