...
object CPersonCenInfoIncTest extends SparkUtil with Until with HbaseUtil{
  def main(args: Array[String]): Unit = {
    ...
    val hbaseData: DataFrame = getHbaseBussValue(sc,"label_person")
      .map(x => {
        ...
        (key,cusType,newCusBuyCun,newCusInsuredCun,newCusSumPremium,policyCun,policyInsuredCun,policyPremium,premiumAvg,lastPolicyEndDate,lastCusType,becomeCurrCusTime)
      })
      .toDF("cert_no","cus_type","new_cus_buy_cun","new_cus_insured_cun","new_cus_sum_premium","policy_cun","policy_insured_cun",
        "policy_premium","premium_avg","last_policy_end_date","last_cus_type","become_curr_cus_time")
  ...
  def getIncCPersonCenInfoDetail(hbaseData:DataFrame,sc:SparkContext,sqlContext: HiveContext) ={
    ...
    /**
      * 读取被保人信息
      */
    val odsPolicyInsuredDetail = sqlContext.sql("select policy_id as policy_id_insured,insured_cert_no,start_date,end_date,work_type,company_name from odsdb.ods_policy_insured_detail")
    ...
    /**
      * 新客阶段和老了阶段累计参保次数
      */
    getInsuredCount(sqlContext,odsPolicyInsuredDetail,policyHolderData,hbaseData)

    /**
      * 新客阶段和老了阶段累计贡献保费和累计购买次数和件均保费 以及新客阶段购买次数和贡献保费
      */
    getbuysAndPremoums(sqlContext,policyHolderData,hbaseData)
    ...
    /**
      ...
      */
    lastStartDateInfo(sqlContext,policyHolderData)
    ...
    /**
      ...
      */
    insuredProfessionInfo(sqlContext,dwPolicyDetailInc,odsPolicyInsuredDetail,odsWorktypeDimension)
  ...
  /**
    ...
    */
  def insuredProfessionInfo(sqlContext:HiveContext,dwPolicyDetailInc:DataFrame,odsPolicyInsuredDetail:DataFrame,odsWorktypeDimension:DataFrame) = {
    import sqlContext.implicits._
    ...
    val res = insuredDetail.join(odsWorktypeDimension,insuredDetail("work_type")===odsWorktypeDimension("work_type_slave"),"leftouter")
      ...
      .map(x => {
        ...
        val nowProfessionRiskLevel = x.getAs[String]("now_profession_risk_level")
        if(endDate == null){
          endDate = startDate
        }
  ...
  /**
    ...
    */
  def getbuysAndPremoums(sqlContet:HiveContext,policyHolderData:DataFrame,hbaseData:DataFrame) ={
  ...
  /**
    ...
    * @param hbaseData hbase数据
    */
  def getInsuredCount(sqlContext:HiveContext,odsPolicyInsuredDetail:DataFrame,policyHolderData:DataFrame,hbaseData:DataFrame) = {
    import sqlContext.implicits._
    /**
      * 参保次数
      */
    val insuredCount = policyHolderData.join(odsPolicyInsuredDetail,policyHolderData("policy_id")===odsPolicyInsuredDetail("policy_id_insured"))
      .where("insured_cert_no = holder_cert_no")
      ...
      .reduceByKey(_+_)
      .map(x => (x._1,x._2))
      .toDF("holder_cert_no","insured_count")

    val hbaseInsuredCount = insuredCount.join(hbaseData,insuredCount("holder_cert_no")===hbaseData("cert_no"),"leftouter")
      .map(x => {
        val holderCertNo = x.getAs[String]("holder_cert_no")
        val certNo = x.getAs[String]("cert_no")
        ...
        (holderCertNo,policyInsuredCun,newCusInsuredCun)
      })
  ...
  /**
    ...
    */
  def getNewInfoDate(sqlContext: HiveContext,policyHolderData:DataFrame,hbaseData:DataFrame) ={
  ...
  /**
    ...
    */
  def getPrePremiumAndRiskCount(sqlContext:HiveContext,odsClaimsDetail:DataFrame) = {
   import sqlContext.implicits._
    /**
      * 累计出险次数
      * 累计赔付保费
  ...
  /**
    ...
    */
  def lastStartDateInfo(sqlContext:HiveContext,policyHolderData:DataFrame) = {
    import sqlContext.implicits._
    /**
      * 起期最大的保单信息
      */
    val res = policyHolderData
      .map(x => {
        ...
        val province = x.getAs[String]("province")
        val city = x.getAs[String]("city")
        var policy_start_date = x.getAs[String]("policy_start_date")
        val policy_end_date = x.getAs[String]("policy_end_date")
        if(policy_start_date == null){
          policy_start_date = policy_end_date
        }
        (holderCertNo,(productCode,productName,province,city,policy_start_date))
      })
      .reduceByKey((x1,x2) => {
  ...
  /**
    ...
    */
  def getPolicyEndDate(sqlContext:HiveContext,policyHolderData:DataFrame,hbaseData:DataFrame) = {
    import sqlContext.implicits._
    //保险止期
  ...
  /**
    ...
    * @param odsClaimsDetail 理赔数据集
    */
  def getRrejectClaimCunAndWithdrawClaimCun(sqlContext:HiveContext,odsClaimsDetail:DataFrame)={
    import sqlContext.implicits._
    /**
      * 拒赔案件数
      * 撤案案件数
