...
//        结果
  ...
  def websiteInfo(hiveContext: HiveContext): DataFrame = {
    ...
    /**
      * 读取holder的hive表
      */
    val holderInfoTemp: DataFrame = hiveContext.sql("select holder_cert_no, holder_cert_type, mobile, policy_id odsdb.ods_holder_detail")
...
//    结果
  ...
  def interInfo(hiveContext: HiveContext): DataFrame = {
    ...
    /**
      * 读取58速运接口的手机号信息
      */
    val suyunTelInfo: DataFrame = hiveContext.sql("select courier_card_no, courier_mobile, start_date, end_date from odsdb_prd.open_express_policy")
      .where("length(courier_card_no) = 18")
      .selectExpr("courier_card_no as cert_no", "courier_mobile as mobile", "start_date", "end_date", "express() as source_type")
...
//    结果
  ...
  def getMobile(list: mutable.ListBuffer[(String, String, String)]): JSONObject = {
    val hashMap = new util.HashMap[String, String]()
    val mobileJSON = new JSONObject(true)
//    循环
    for (l <- list) {
      hashMap.put(l._2, l._1)
    }
  ...
  def getSource(list: mutable.ListBuffer[(String, String, String)]): String = {
    var maxDate: String = "2016-01-01 00:00:00"
    var maxDateSource: String = null
//    循环遍历
    for (l <- list) {
      if (l._1 > maxDate) {
        maxDate = l._1
