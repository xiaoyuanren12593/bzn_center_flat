...
/**
  ...
  * describe: 费雇主电子台账
  **/
object DwUnEmpTAccountIntermediatePreserveDetailTest extends SparkUtil with Until{
  ...
  def getAllBusinessPolicyDetail(sqlContext:HiveContext) = {
    ...
    /**
      *获取体育保单数据信息
      */
    val resPreserve = getSportsScenMemberPreserveDetail(sqlContext,policyAndPlanAndTeamRes,odsPreservationDetail)
    resPreserve.printSchema()
  }
  ...
  /**
    ...
    * @return
    */
  def getSportsScenMemberPreserveDetail(sqlContext:HiveContext,policyAndPlanAndTeamRes:DataFrame,odsPreservationDetail:DataFrame): DataFrame = {
    ...
    policyAndPlanAndTeamAndProductPreserveRes.show()
