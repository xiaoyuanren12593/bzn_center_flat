...
import bzn.dw.bclickthrough.DwEmpTAccountsIntermediateDetailTest.{clean, readMysqlTable, sparkConfInfo}
...
object DwEmpTAccountsIntermediateDetailTest16_19 extends SparkUtil with Until with MysqlUntil {
  ...
  def TAccountsEmployerAddPreserve(hqlContext: HiveContext, sqlContext: SQLContext): DataFrame = {
    ...
    val resTempRes = preserveAndsaleAndTeam.join(odsPolicyProduct, 'insure_code === 'product_code, "leftouter")
      ...
      .where("policy_status in (0,1,-1) and preserve_status = 1 " +
