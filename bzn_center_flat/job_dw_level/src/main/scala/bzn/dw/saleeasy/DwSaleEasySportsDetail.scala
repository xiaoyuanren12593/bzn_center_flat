...
object DwSaleEasySportsDetail  extends  SparkUtil with Until with DataBaseUtil{
  ...
  def main(args: Array[String]): Unit = {
    ...
    if(data > 0){
      val dataMonitor =
        Seq(
          DmbBatchingMonitoringDetail(nowTime+"`dm_saleeasy_sports_detail`"+"clickhouse","clickhouse","销售易体育数据清洗","dw","odsdb","dm_saleeasy_sports_detail",1,"销售易体育数据清洗-成功",new Timestamp(System.currentTimeMillis()),new Timestamp(System.currentTimeMillis()))
        ).toDF()
