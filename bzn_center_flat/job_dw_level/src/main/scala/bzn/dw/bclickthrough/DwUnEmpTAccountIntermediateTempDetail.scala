...
object DwUnEmpTAccountIntermediateTempDetail extends SparkUtil with Until {
  def main (args: Array[String]): Unit = {
    ...
    hiveContext.sql ("truncate table dwdb.dw_t_accounts_un_employer_intermediate_detail")
  ...
  def getAllBusinessPolicyDetail (sqlContext: HiveContext): DataFrame = {
    ...
    resPolicy.repartition(10).write.mode(SaveMode.Append).saveAsTable("dwdb.dw_t_accounts_un_employer_intermediate_detail")
    val resPreserve = getSportsScenMemberPreserveDetail (sqlContext, policyAndPlanAndTeamRes, odsProductDetail, odsPreservationDetail)
    resPreserve.repartition(10).write.mode(SaveMode.Append).saveAsTable("dwdb.dw_t_accounts_un_employer_intermediate_detail")
    val res = resPolicy.unionAll(resPreserve)
    res
  }
