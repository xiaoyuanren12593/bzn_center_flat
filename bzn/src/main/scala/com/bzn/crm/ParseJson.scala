...
//    res1.foreach(println)
  ...
  def getBaseInfo(value : Array[AnyRef]): Array[String] ={
    var baseInfo: Array[String] = value.map(x => {
      ...
      val updateUser = JSON.parseObject(JSON.parseObject(x.toString).get("updateUser").toString).get("realname")//最近修改人
      val master = JSON.parseObject(JSON.parseObject(x.toString).get("master").toString).get("realname")//客户所有人
      val masterOffice = JSON.parseObject(JSON.parseObject(x.toString).get("masterOffice").toString).get("name")//客户所属部门
      val createTime = JSON.parseObject(x.toString).get("createTime")//创建时间
      val updateTime = JSON.parseObject(x.toString).get("updateTime")//最近修改时间
